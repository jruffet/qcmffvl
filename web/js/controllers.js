...
angular
  ...
  .controller("MainCtrl", [
    ...
    "dialogs",
    "$window",
    function ($scope, $location, $timeout, $http, $filter, $localStorage, dialogs, $window) {
      ...
      $scope.loadJSON = function () {
        ...
        return $timeout(function () {
          return $http
            .get("./generated/qcm_ffvl.json?v=" + $scope.qcmVersion)
      ...
      $scope.updateQCMID = function () {
        $scope.main.QCMID = QCM.QCMID(
          $scope.$storage.conf.seed,
          $scope.optionsToArray(),
          $scope.version,
          $scope.qcmVersion,
        );
      ...
      $scope.checkForUpdates = function () {
        ...
        if (navigator.onLine) {
          $http.get("./json/versions.json?v=" + new Date().getTime()).then(function (resp) {
      ...
      $scope.reload = function () {
        ...
        const categoryText = category.indexOf("Toutes") === -1 ? ` de la catégorie <b>${category}</b>` : "";
      ...
      $scope.$watchGroup(
        [
          "$storage.conf.nbquestions",
          "$storage.conf.level",
          "$storage.conf.category",
          "$storage.conf.activity",
        ],
      ...
      $scope.dialogShare = function () {
        dialogs
          .create("qcmid.html", "QCMIDDialogCtrl", $scope.main, { size: "lg" })
      ...
      $scope.dialogParameters = function () {
        dialogs
          .create("parameters.html", "ParametersCtrl", $scope, { size: "lg" })
          .result.then(function (changedKeys) {
      ...
      $scope.loadJSON().then(function () {
        ...
        $http.get("./json/changelog.json?v=" + $scope.version).then(function (resp) {
          $scope.changelog = resp.data;
        });
        $http.get("./json/thanks.json?v=" + $scope.version).then(function (resp) {
          $scope.thanks = resp.data;
        });
        ...
        if (!wasLoadPath) {
          if (Object.keys($scope.$storage.answers).length > 0) {
            ...
            const dlg = dialogs.confirm(
              "Chargement du dernier QCM",
              "Charger le dernier questionnaire inachevé (avec vos réponses) ?",
            );
            dlg.result
              .then(
                function () {
  ...
  .controller("QCMCtrl", [
    ...
    function ($scope) {
      ...
      $scope.toggleCheck = function (q, answer) {
        if (
          $scope.navCollapsed &&
          !$scope.main.checkAnswers &&
          !$scope.main.exam.enabled &&
          !$scope.isHelpQuestion(q)
        ) {
